...
CREATE TABLE IF NOT EXISTS `mydb`.`project` (
  ...
  `employee_emp_num` INT NOT NULL,
  PRIMARY KEY (`proj_num`, `employee_emp_num`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`project_has_suppliers` (
  `project_proj_num` INT NOT NULL,
  `suppliers_supl_num` INT NOT NULL,
  PRIMARY KEY (`project_proj_num`, `suppliers_supl_num`),
  INDEX `fk_project_has_suppliers_suppliers1_idx` (`suppliers_supl_num` ASC) VISIBLE,
  INDEX `fk_project_has_suppliers_project1_idx` (`project_proj_num` ASC) VISIBLE,
  CONSTRAINT `fk_project_has_suppliers_project1`
    FOREIGN KEY (`project_proj_num`)
    REFERENCES `mydb`.`project` (`proj_num`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_project_has_suppliers_suppliers1`
    FOREIGN KEY (`suppliers_supl_num`)
    REFERENCES `mydb`.`suppliers` (`supl_num`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
